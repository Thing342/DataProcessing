...
if args.errorcheck:
    ...
    first = True
    for region in list(s.active_preview_mileage_by_region.keys()):
        if not first:
            sqlfile.write(",")
        first = False
        active_only_mileage = 0.0
        active_preview_mileage = 0.0
        if region in list(s.active_only_mileage_by_region.keys()):
            active_only_mileage = s.active_only_mileage_by_region[region]
        if region in list(s.active_preview_mileage_by_region.keys()):
            active_preview_mileage = s.active_preview_mileage_by_region[region]
    ...
    for start in range(0, len(s.ccr_values), 10000):
        sqlfile.write('INSERT INTO clinchedConnectedRoutes VALUES\n')
        first = True
        for line in s.ccr_values[start:start + 10000]:
            if not first:
                sqlfile.write(",")
            first = False
            sqlfile.write(line + "\n")
    ...
    for start in range(0, len(s.cr_values), 10000):
        sqlfile.write('INSERT INTO clinchedRoutes VALUES\n')
        first = True
        for line in s.cr_values[start:start + 10000]:
            if not first:
                sqlfile.write(",")
            first = False
            sqlfile.write(line + "\n")
